...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	git clone --depth 1 --no-single-branch https://github.com/KazuDante89/AnyKernel3-EAS -b lavender2
}
...
tg_post_msg() {
	curl -s -X POST "$BOT_MSG_URL" -d chat_id="-1001293242785" \
...
gen_zip() {
	...
	mv "$KERNEL_DIR"/out/arch/arm64/boot/Image.gz-dtb AnyKernel3-EAS/Image.gz-dtb
	if [ $BUILD_DTBO = 1 ]
	then
		mv "$KERNEL_DIR"/out/arch/arm64/boot/dtbo.img AnyKernel3-EAS/dtbo.img
	fi
	cd AnyKernel3 || exit
